#!/usr/bin/env node
/**
 * Renames any price variant whose name contains "juneteenth" (case-insensitive)
 * to "Yacht Party | Friday July 3rd" across all listings via the Sharetribe Integration API.
 *
 * Dry run (no changes): node scripts/remove-juneteenth-variant.js
 * Apply changes:        node scripts/remove-juneteenth-variant.js --apply
 */
...
const NEW_NAME = 'Yacht Party | Friday July 3rd';

const isJuneteenth = name =>
  typeof name === 'string' && name.toLowerCase().includes('juneteenth');
...
(async () => {
  ...
  for (const listing of listings) {
    ...
    const needsChange = variants.some(v => isJuneteenth(v.name));
    if (!needsChange) continue;

    const updated = variants.map(v =>
      isJuneteenth(v.name) ? { ...v, name: NEW_NAME } : v
    );

    console.log(`  [${listing.id.uuid}] "${listing.attributes.title}"`);
    variants.forEach((v, i) => {
      if (isJuneteenth(v.name)) {
        console.log(`    variant[${i}]: "${v.name}" → "${NEW_NAME}"`);
      }
    });

    if (APPLY) {
      await updateListing(token, listing.id, { priceVariants: updated });
      console.log(`    ✅ saved`);
    }
    updatedCount++;
  }
  ...
  if (updatedCount === 0) {
    console.log('No Juneteenth variants found in any listing.');
  } else {
    console.log(
      `\n${APPLY
        ? `Updated ${updatedCount} listing(s).`
        : `${updatedCount} listing(s) would be updated. Run with --apply to save.`}`
    );
  }
